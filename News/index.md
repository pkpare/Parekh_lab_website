---
title: Lab News
nav:
  order: 3
  tooltip: Updates
---

# {% include icon.html icon="fa-solid fa-feather-pointed" %}Lab News


{% include section.html %}

{% capture text %}
May 2026 - Parekh lab axe throwing outing to celebrate a productive semester.<br>



<br>

{:.center}
{% endcapture %}

{%
  include feature.html
  image="images/labouting2026.png"
  headline=""
  text=text
%}

{% capture text %}
May 2026 - Dr. Parekh and Alexa Dinklang attend the Society of Biological Psychiatry (SOBP) annual meeting in NYC. Alexa presented a poster.<br>



<br>

{:.center}
{% endcapture %}

{%
  include feature.html
  image="images/AlexaSOBPcrop3.png"
  headline=""
  text=text
%}

{% capture text %}
April 2026 - Aryan Bangad wins 2nd place (out of nearly 100 participants) in the URSA poster competition. His worked was judged by academic and industry professionals.<br>



<br>

{:.center}
{% endcapture %}

{%
  include feature.html
  image="images/Aryan_URSA_pic.jpeg"
  headline=""
  text=text
%}


{% capture text %}
March 2026 - Celebrating Austin Csiszar's acceptance to the Behavioral Neuroscience PhD Program at University of Colorado Boulder.<br>



<br>

{:.center}
{% endcapture %}

{%
  include feature.html
  image="images/Austinsendoff2.png"
  headline=""
  text=text
%}

{% capture text %}
December 2025 - Annual holiday party and white elephant gift exchange with members from Eagle, Gu and Lakshminarasimhan labs.<br>



<br>

{:.center}
{% endcapture %}

{%
  include feature.html
  image="images/holiday_2025.png"
  headline=""
  text=text
%}

{% capture text %}
November 2025 - Ausin Csiszar and Alexa Dinklang present posters at the ABRCMS annual meeting in San Antonio, Texas<br>



<br>

{:.center}
{% endcapture %}

{%
  include feature.html
  image="images/ABRCMSgrouppic.png"
  headline=""
  text=text
%}

{% capture text %}
October 2025 - Dr. Parekh is awarded a NARSAD Young Investigator grant from the Brain and Behavior Research Foundation (BBRF) to support lab projects. <a href="https://news.utdallas.edu/faculty-staff/accolades-nov-2025/">Read more here</a>.<br>



<br>

{:.center}
{% endcapture %}

{%
  include feature.html
  image="images/BBRFlogo2.png"
  headline=""
  text=text
%}

{% capture text %}
August 2025 - MACS lab undergraduates present posters at the SPUR symposium - Aryan Bangad wins a Provost's Office Award<br>



<br>

{:.center}
{% endcapture %}

{%
  include feature.html
  image="images/AryanSPUR.png"
  headline=""
  text=text
%}

{% capture text %}
June 2025 - Dr. Parekh attends and Chairs a panel at the International Behavioral Neuroscience Society (IBNS) annual meeting in Tromso, Norway<br>



<br>

{:.center}
{% endcapture %}

{%
  include feature.html
  image="images/NorwayIBNS.png"
  headline=""
  text=text
%}

{% capture text %}
May 2025 - Pranavi Maddipatla graduates with her Bachelor's degree in Neuroscience and stays on as an ACN Masters student with the lab<br>



<br>

{:.center}
{% endcapture %}

{%
  include feature.html
  image="images/Pranavi_grad.png"
  headline=""
  text=text
%}


{% capture text %}
April 2025 - Dr. Parekh wins a Travel Fellowship Award to attend and present at the Society of Biological Psychiatry (SOBP) annual meeting in Toronto, Canada<br>



{:.center}
{% endcapture %}

{%
  include feature.html
  image="images/SOBP_2025_pic.jpg"
  headline=""
  text=text
%}

{% capture text %}
March 2025 - The lab receives funding through an R00 award from the National Institute of Mental Health (NIMH). <a href="https://research.utdallas.edu/blog/dr-puja-parekh-awarded-over-740000-from-nih">Read more here</a>.<br>



<br>

{:.center}
{% endcapture %}

{%
  include feature.html
  image="images/nimh-logo2.png"
  headline=""
  text=text
%}

{% capture text %}
February 2025 - Review article published in the journal, Neurbiology of Stress<br>



<br>

{:.center}
{% endcapture %}

{%
  include feature.html
  image="images/NBS_2025.jpg"
  headline=""
  text=text
%}


{% capture text %}
December 2024 - Holiday white elephant gift exchange<br>



<br>

{:.center}
{% endcapture %}

{%
  include feature.html
  image="images/holidayparty.png"
  headline=""
  text=text
%}

{% capture text %}
August 2024 - The Parekh lab officially opens on the third floor of BSB!<br>



<br>

{:.center}
{% endcapture %}

{%
  include feature.html
  image="images/labopening.png"
  headline=""
  text=text
%}
